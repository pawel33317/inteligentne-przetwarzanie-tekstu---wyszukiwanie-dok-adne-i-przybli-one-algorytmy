...
public class Main {
	...
	//tylko bez debugu operuje na 500Kilo danych
	//public static final String FILE_MAIN = ".prepared500";
		...
		for (String mainFile : mainFiles) {// dla kazdego pliku g³ównego: dna,
			...
			for (String patternFile : patternsList) {// dla kazdego wzorca:
				...
				while (previousCurrentSearch != currentSearch) {
					...
					int ileLiterSprawdzicMaksymalnie = ileLiterSprawdzicMaksymalnieFun(pattern.length(), suffuxTable[currentSearch].value);
					int wynikPorownania = czyWzorzecJestRowny(pattern, ileLiterSprawdzicMaksymalnie, suffuxTable[currentSearch].value);
					// Trafienie
					if (wynikPorownania == 0) {
						...
						while (tmpCurrentSearch < wejscie.length && czyWzorzecJestRowny(pattern, ileLiterSprawdzicMaksymalnieFun(pattern.length(), suffuxTable[tmpCurrentSearch].value), suffuxTable[tmpCurrentSearch].value) == 0) {
							trafieniaSUF++;
							tmpCurrentSearch++;
						}
						while (tmp2CurrentSearch >= 0 && czyWzorzecJestRowny(pattern, ileLiterSprawdzicMaksymalnieFun(pattern.length(), suffuxTable[tmp2CurrentSearch].value), suffuxTable[tmp2CurrentSearch].value) == 0) {
							trafieniaSUF++;
							tmp2CurrentSearch--;
						}
						break;
				...
				SUFtime_SEARCH_ONLY = (System.nanoTime() - startTimeSearchOnly);
				// BRAK TRAFIENIA
